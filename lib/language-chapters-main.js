...
module.exports = {
  ...
  /**
   * Command: language-chapters:lookup
   *
   ...
   * a function known to chapters, in a web browser or the root functions page
   * if the rest is unavailable.
   */
  lookupSelected: function() {
    console.log("[language-chapters] lookup invoked!");
    this._getSelectedText(this.chaptersFnRegEx, (function(text) {
      var browser;

      browser = atom.packages.getActivePackage('browser-plus');
      if (browser) {
        if (text.length) {
          console.log('[language-chapters] Selected function -> %o', text);
          atom.workspace.open([this.urlBase, text].join(''));
        }
        else {
          console.log('[language-chapters] Opening functions root');
          atom.workspace.open(this.fnBase);
        }
      }
      else {
        console.error("[language-chapters] browser-plus is a required package.");
      }
    }).bind(this));
  },
  ...
  updateGrammars: function() {
    ...
    if (!grammar || !grammar.rawPatterns) {
      setTimeout(this.updateGrammars.bind(this), 1000);
      console.log('[language-chapters] Grammar not yet available, trying again in 1s');
      return;
    }
    ...
    var patterns = [
      [true, 'keyword.chapters-keyword', [language.keywords]],
      // [true, 'keyword.control.chapters.blocktype', [language.blocktypes, true]],
      // [true, 'keyword.chapters.baseFns', [language.chaptersFns]],
      // [cfg.highlightFOSE, 'keyword.chapters.foseFns', [language.foseFns]],
      // [cfg.highlightVegas, 'keyword.chapters.vegasFns', [language.vegasFns]],
      // [cfg.highlightNVSE, 'keyword.chapters.nvseFns', [language.nvseFns]],
      // [cfg.highlightNX, 'keyword.chapters.nxFns', [language.nxFns]],
      // [cfg.highlightLutana, 'keyword.chapters.lutanaFns', [language.lutanaFns]],
      // [cfg.highlightJIP, 'keyword.chapters.jipFns', [language.jipFns]]
    ];
